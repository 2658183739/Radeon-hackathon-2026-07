...
`mobile_bimanual/self_improvement_v3/` 是本次失败驱动周期的紧凑哈希证据。它绑定七回合专家
课程、4,557 帧数据审计、2,800 步 Radeon 训练日志、42 阶段 Harness-Lite 消融、数据隔离审计、
基线/候选各 100 回合配对评估、Wilson 区间和失败闭合晋级决策。

保留的 v2 checkpoint 为 96/100；桥接训练后的 v3 为 91/100，因未通过已注册的 Wilson 不劣性门
而没有晋级。两组均无 35 N 力越界，并且每个回合都有 VLA 实际执行和单 Radeon/ROCm 证据。
结果只覆盖仿真中四类刚性包裹的参数变化，不代表未见几何体或 Sim-to-Real。

## 移动 RGB-D v1 配对消融

`mobile_bimanual/rgbd_v1/` 绑定 4,557 帧 RGB-D 数据审计、2,800 步 Radeon 日志、42 阶段配对消融、
自动模态门、一个在线开发集成回合和远端 checkpoint 哈希。RGB-D 链路可用但没有改善 Harness MAE，
因此继续选择 RGB，不作广泛 RGB-D 任务成功率声明。

## 移动双臂三吸盘/V 型托架协同

`mobile_bimanual/cooperative_cradle_v1/comparison.json` 绑定固定 `1.44 x 0.12 x 0.18 m` 超长
刚性纸箱的 v19-v24 配对开发链。v20 证明专家双臂抬升、30 cm 运输、落位和释放机理；v21-v23
依次保留 VLA 进度不足和截止门过晚的负结果；v24 使用从独立专家执行校准的 0.60 容量系数完成
任务，最终误差 2.16 cm、零断吸、托架峰值力 30.65 N。SmolVLA 实际执行 647 个物理步，随后
截止门单向交还专家。`REMOTE_SHA256SUMS` 记录七个远端完整 summary 的哈希。

PASH-VLA 的 `pash-v25-summary.json` 在同一对象上开启 Force-Memory：68 次推理触发 11 次
收紧和 2 次完全回退，任务成功，最终误差 2.11 cm，托架峰值 30.64 N。该结果只用于方法
回归，未打开新的统计测试集。
