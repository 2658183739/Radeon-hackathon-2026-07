...
% Venue-neutral double-blind draft. The ICLR 2027 author guide is live, but its
% linked iclr2027.zip was absent from the official template repository on
% 2026-07-29. Replace only this preamble when the archive becomes retrievable;
% do not change evidence text during the format migration.
...
factor under paired initial states. Figure~\ref{fig:system-overview} summarizes
the learned-policy boundary, evidence ladder, and deployment measurements.

The paper is organized around three questions. First, can a protocol-matched
candidate improve over the locally reproduced $\pi_{0.5}$ LIBERO control
without using privileged state? Second, can the same methodology produce
reliable pure-VLA
...
\begin{figure*}[t]
  ...
  \caption{Contract-gated $\pi_{0.5}$ adaptation and Radeon deployment. The
  learned policy owns the declared task actions, while deterministic execution
  code performs coordinate transforms, interpolation, hard-limit enforcement,
  and rejection without expert completion. Candidates advance from action
  fidelity through paired development to one-shot confirmation; latency, memory,
  energy, and thermal evidence are reported separately for the persistent Radeon
  runtime.}
...
\end{equation}
while leaving the existing categorical mode-head coefficient and class weights
unchanged and outside the stage multiplier. The stage weights are normalized so
that their dataset-frequency-weighted mean equals one. This preserves the overall
gradient scale and isolates the effect of emphasizing contact-critical stages
from changes to the already trained mode decoder.
...
experiments record warm-latency P50, P95, and P99, peak memory, package energy,
...
contract, and B3 replaces it with bounded incremental SE(3). After B3 fails its
action-fidelity gate, B3-SW returns to the B2 absolute contract and changes only
the normalized stage weighting of the continuous flow loss. B4 adds verified
recovery replay, and B5 adds short observation memory and explicit policy
metadata. Runtime
...
confirmation rollouts (96.25\%, Wilson 95\% CI: 93.91--97.71\%). Suite success
was 98\% on LIBERO-Spatial, 99\% on LIBERO-Object, 96\% on LIBERO-Goal, and
92\% on LIBERO-10. The published OpenPI aggregate is 96.85\%~\citep{openpi};
it is not paired with the local run and is therefore reported only as context.
Table~\ref{tab:libero} reserves the candidate row until development selection
and one-shot confirmation are complete.
...
\paragraph{Full paired development.}
The selected eight-action candidate then completed 385/400 development units
(96.25\%, Wilson 95\% CI: 93.91--97.71\%), compared with 384/400 for the
ten-action control (96.00\%, 93.60--97.52\%). The paired outcomes contained 13
candidate wins and 12 losses, a 0.25 percentage-point difference (paired
Newcombe 95\% CI: $-2.33$ to $2.85$; exact one-sided McNemar $p=0.5$). This
satisfied the pre-registered strict-win selection rule and froze the eight-action
candidate for one-shot confirmation, but it did not establish superiority. Its
warm model P95 was 0.50\% lower while package energy was 14.58\% higher than the
control, so the cadence change is not presented as a Radeon efficiency gain.
...
The one-shot LIBERO candidate confirmation, B3-SW action gate, frozen parcel
confirmation, verified-replay
...
methodological motivation. The frozen parcel confirmation and subsequent
physical-robot replication should test the first limitation. Future data
collection should expand independent operators, sessions, and underrepresented
contact modes before increasing frame count. A direct $\pi_{0.6}$ comparison can
be added when public weights and a compatible action adapter become available,
using a newly frozen confirmation population rather than reopening the present
one.
...
OpenAI Codex was used to assist with experimental-design review, software
implementation and testing, evidence organization, literature retrieval,
statistical summarization, and drafting and editing this manuscript. All
AI-assisted code and numerical claims were checked through executable tests,
frozen manifests, and machine-readable artifacts. \texttt{TBD: final author
confirmation of manual review and responsibility before submission.}

\section*{Reproducibility Statement}

Every reported result is tied to versioned configurations, dataset and
checkpoint hashes, action contracts, random seeds, raw summaries, paired
statistical tests, and ROCm telemetry. The anonymized supplementary release will
include the executable evaluation and training entry points together with the
frozen manifests needed to reproduce each claim. Cells marked \texttt{TBD} are
not treated as results and will be replaced only from the corresponding
