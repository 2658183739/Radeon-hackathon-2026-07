...
experience in the $\pi_{0.6}^{*}$ system~\citep{recap}. No public $\pi_{0.6}$
checkpoint and matched LIBERO adapter are available in the executable stack used
here, so $\pi_{0.6}$ motivates verified recovery experiments but is not treated
as a numerical baseline.
...
and retry index. Missing sensors are masked explicitly. The action contract
contains base motion, bounded world-frame SE(3) increments for both arms, tool
commands, a learned grasp mode, and task progress. Rotations use shortest-path
rotation vectors with a fixed composition order, and executable round-trip tests
cover normalization, denormalization, decoding, bounds, and phase transitions.
...
be interpreted as adaptation under limited behavioral coverage. Third, no public
$\pi_{0.6}$ weights and matched evaluation adapter are available in the current
stack; claims of numerical superiority to $\pi_{0.6}$ would therefore be
unverifiable. The final paper will compare directly with executable $\pi_{0.5}$
baselines and discuss $\pi_{0.6}^{*}$ only as methodological motivation.
