...
草稿只写入已经有机器证据的结果：本地 PI0.5 LIBERO 400 回合为 96.25%，Radeon eager 长稳通过，max-autotune 因成功与能效门失败而被拒绝，B3 增量 SE(3) 因动作保真回退未晋级；action-step 开发筛选选择了 8-step（40/40）。完整 400+400 开发配对中，8-step 为 385/400，10-step 为 384/400，满足预注册的严格胜出晋级规则，但配对单侧 McNemar `p=0.5`，不支持统计优越性声明；唯一一次确认正在运行。LIBERO 最终候选、B3-SW、快递冻结闭环和验证式恢复仍标为 `TBD`。
