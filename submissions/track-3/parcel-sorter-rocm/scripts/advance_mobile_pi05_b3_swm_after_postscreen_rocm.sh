#!/usr/bin/env bash
# Select B3-SWM with its frozen paired gate, then run strict pure-VLA development.
...
RUN_NAME="${4:-pi05-b3-swm-stage-mode-weighted}"
RUN_ID="${5:-b3-swm}"
CONFIG="${ROOT_DIR}/configs/mobile_pi05_b3_swm_stage_mode_weighted_flow_v1.json"
CONTROL_SCREEN="/workspace/persistence/parcel-sorter-opt-v1/outputs/pi05-b3-sw-stage-weighted-common-seed-v3/pi05-b3-sw-stage-weighted-step12000-six-observation-screen.json"
SELECTION="${SCREEN_ROOT}/B3_SWM_PAIRED_ACTION_FIDELITY_SELECTION.json"
...
if [[ -e "${SELECTION}" || -e "${STRICT_OUTPUT}" ]]; then
  echo "ERROR: B3-SWM selection or strict-development output already exists" >&2
  exit 3
fi
...
test -f "${CONFIG}" || {
  echo "ERROR: missing frozen B3-SWM preregistration: ${CONFIG}" >&2
  exit 4
}
test -f "${CONTROL_SCREEN}" || {
  echo "ERROR: missing frozen B3-SW control screen: ${CONTROL_SCREEN}" >&2
  exit 4
}
...
for step in 3000 6000 9000 12000; do
  ...
  if [[ ! -f "${screen}" ]]; then
    echo "ERROR: missing completed B3-SWM screen: ${screen}" >&2
    exit 5
  fi
  diagnostic="${SCREEN_ROOT}/${RUN_NAME}-step${step}-high-noise-mode-probes.json"
  if [[ ! -f "${diagnostic}" ]]; then
    echo "ERROR: missing completed B3-SWM high-noise diagnostic: ${diagnostic}" >&2
    exit 5
  fi
...
if payload.get("status") != "promoted" or len(selected) != 1:
    raise SystemExit("B3-SWM selection is not uniquely promoted")
