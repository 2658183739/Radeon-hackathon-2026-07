...
OUTPUT_DIR="${2:-/root/pi05-runs/mobile-pi05-b1-droid-absolute-12000-v1}"
LOG_PATH="${3:-${ROOT_DIR}/outputs/pi05-b1-droid-absolute-train-v1.log}"
...
if [[ -e "${OUTPUT_DIR}" || -e "${LOG_PATH}" || -e "${PID_PATH}" ]]; then
  echo "ERROR: B1 output, log, or PID file already exists" >&2
  exit 2
fi
...
nohup env \
  ...
  MOBILE_PI05_FULL_ACTION_PROJECTIONS=0 \
...
if ! kill -0 "${training_pid}" 2>/dev/null; then
  echo "ERROR: B1 training exited during launch; inspect ${LOG_PATH}" >&2
  exit 4
fi
