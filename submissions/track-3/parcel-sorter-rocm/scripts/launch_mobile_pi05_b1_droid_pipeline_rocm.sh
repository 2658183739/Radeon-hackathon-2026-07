...
RUN_ROOT="/root/pi05-runs/mobile-pi05-b1-droid-absolute-12000-v1"
TRAIN_LOG="${ROOT_DIR}/outputs/pi05-b1-droid-absolute-train-v1.log"
SCREEN_ROOT="${ROOT_DIR}/outputs/pi05-b1-droid-absolute-common-seed-v1"
POST_LOG="${ROOT_DIR}/outputs/pi05-b1-droid-absolute-postscreen-v1.log"
STRICT_ROOT="${ROOT_DIR}/outputs/pi05-b1-droid-absolute-strict-dev-v1"
AUTO_LOG="${ROOT_DIR}/outputs/pi05-b1-droid-absolute-strict-auto-v1.log"
...
nohup bash "${ROOT_DIR}/scripts/postscreen_mobile_pi05_training_rocm.sh" \
  ...
  "${SCREEN_ROOT}" pi05-b1-droid-absolute 3000 6000 9000 12000 \
...
for pid in "${train_pid}" "${post_pid}" "${auto_pid}"; do
  if ! kill -0 "${pid}" 2>/dev/null; then
    echo "ERROR: B1 pipeline process exited during launch: ${pid}" >&2
    exit 2
  fi
done
