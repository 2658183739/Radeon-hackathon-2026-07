...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for step in range(1, args.steps + 1):
        ...
        unsafe_loss = bce(prediction[:, 0], unsafe_target)
        success_loss = bce(prediction[:, 1], success_target)
        force_loss = smooth_l1(prediction[:, 2], force_target)
        duration_loss = smooth_l1(prediction[:, 3], duration_target)
