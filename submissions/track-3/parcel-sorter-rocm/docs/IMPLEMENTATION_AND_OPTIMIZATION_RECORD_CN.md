...
**完整候选决策。**接线冒烟通过后，下一项有效工作是模型学习，而不是继续添加控制器功能。因此
在同一冻结 4,557 帧数据上，以 batch 8、四个数据进程和单张 Radeon 完成 2,800 次 AMP 更新；
没有增加快递类型、阈值或 holdout 样本。完整日志恰好包含 2,800 条有限更新，前 100 步平均损失
为 1.5761，后 100 步为 0.0733。

**Harness 决策。**训练后原始动作在 42 个 episode-stage 样本上的安全包络通过数仍为 0/42，
确定性裁剪与 Harness 为 42/42，因此不能因模仿损失下降而扩大模型权限。在既有确定性闭环案例中，
10 步参考与 2,800 步候选均成功，放置误差从 1.28 cm 变为 0.90 cm，调用从 79 次变为 69 次。
这些只是配对描述结果；在运行原有冻结晋级门前，正式 checkpoint 不变。

## 已完成 Primitive 晋级与原子激活

**观察。**上面的实现记录刻意写于冻结晋级活动之前。该活动现已完成，若仍把它作为最新状态，
会错误描述当前活动模型。

**决策。**先按完整、有序的物理参数签名配对基线与候选，再执行既有统计门。通过的门必须携带
候选模型 SHA-256，随后才允许原子写入活动模型注册表；候选被拒绝或哈希不匹配时，旧注册表必须
保持逐字节不变。

**证据。**2,800 步 primitive-progress checkpoint 以 94/100 对比 96/100 基线，力越界为零，
100/100 回合均有 VLA 实际执行和单张 Radeon/ROCm 来源，且通过登记的 80% 与配对 Wilson
不劣性门。活动模型哈希为
`b83d5299123e1cfeac3463679e021816f0999799492f6fafea828bf823761482`。
Radeon 定向验证通过 4 项注册表测试、7 项周期测试和 12 项自适应重试测试；primitive 周期干运行
还完整渲染了采集、审计、训练、评估、晋级与激活契约，没有启动新的训练活动。

**边界。**这证明了可审计的回合间自我改进机制和一个已晋级候选；它不证明运行中更新权重、
未见几何泛化、真机迁移，也不把单个已知失败的第二次恢复写成总体恢复率提升。
