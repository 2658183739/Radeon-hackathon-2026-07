...
### 8. 把抓后执行改成可观测、可消融的任务阶段

**起点。** 抓取位姿可行不等于完整搬运可行。原 `MOVE_DROP` 同时抬升、水平运输和下降，只在下降
阶段对规划抓取限速，无法判断冲击究竟发生在哪个子阶段。

**实现步骤。** 先增加默认关闭的运输契约和类型化参数，再为每个阶段加入单向状态、动作目标、末端
和包裹四元数、相对位置遥测。两次轨迹审计发现“连续稳定”被控制振荡反复清零，因此将阶段交接改为
首次到达后的固定等待。最后增加有界参考前瞻：参考点按小步推进，但实际跟踪误差仍受全局步长限制。

**为什么没有继续调参。** 该机制能修复一个 69.00 N 回归并保留一个既有成功，但第三个固定开发回合
在慢速版本中滑落，在前瞻版本中仍以 39.95--40.95 N 中止。继续扫描增量会把控制器拟合到一个已
查看回合，不能形成可推广证据。因此代码作为默认关闭的实验能力保留，学习策略与确认集均不启动。

**下一项可执行研究。** 使用抓后相对位姿/旋转漂移构造滑移标签，比较负载感知候选评分与显式重抓；
若现有 top-down 平行夹爪的静态无碰撞位姿始终只剩高位浅接触，则新增侧抓候选，而不是调整 35 N。
...
本轮在不改变默认控制器的前提下增加三项可复用工程能力：

1. 与仿真入口分离的带载抓取阈值/排序模块，以及使用完整 `SimState` 的诊断 runner；
2. 显式放回和释放状态、有界垂直运动，以及可观测的完成条件；
3. 纯函数实现、可单测的重试黑名单语义，并在新开关关闭时保留历史行为。

实现顺序遵循当前项目的优化规范：先消除候选之间隐藏的场景状态延续，再用已知完整运输失败检查
预测有效性；随后只改变一个物理恢复机制，最后独立检查候选反馈。这样可以避免把“状态转换成功”
误写为“任务成功”，也避免把无关物理变化归功于排序调整。

Radeon 全量 216 项测试通过。短时 rollout 没有通过预测验证，两条重抓路径也都在第二次运输中超过
未修改的 35 N 安全线。代码和证据为诊断及未来组合保留，但所有新运行时开关继续关闭。下一项优化
必须改变支撑几何，或使用更长的带载时域；实测结果不支持继续局部扫描高度、夹力和步长。

## 已完成的完整时域反事实决策

本轮增加四项默认中性的能力：有界笛卡尔分段、显式完整时域完成门禁、确定性完整任务候选排序，
以及执行前诊断候选 allowlist。allowlist 会写入规划事件，并通过已单测的纯函数与正常拒绝集合合并；
正式路径得到 `None`，因此历史选择行为不变。

实验刻意把模型误差与控制器误差分开。理想化路径覆盖了更多物理阶段，却仍绕过正式执行动态，因而
验证失败；正式反事实保留这些动态，并找到了安全成功的替代候选。这说明执行保真度属于标签定义，
不只是实现细节。

代码被接受用于数据采集和诊断。由于正结果只来自一个已观察 episode，正式排序调整继续暂缓。
下一优化是用静态几何、IK/可操作度、包裹属性、目标几何和正式闭环结果，在 ROCm 上训练小型候选
评分器。安全中止作为硬目标，35 N 监督器继续放在模型之外。

最终验证显式使用当前源码目录，编译 `src`、`scripts` 与 `tests` 后，Radeon 全量 227 项测试通过。

## 已实现的结构化评分器边界

学习组件被刻意限制：纯提取层验证每个特征与正式标签；网络模块只负责架构和检查点加载；训练入口
只负责优化；评估入口负责候选级选择和延迟。控制器不导入这些组件。这样可以继续做数据与检查点
工作，而不会静默改变任务行为或削弱确定性安全。

smoke 在完整接入前发现并修正了一个共享排序函数边界错误。冷启动始终与预热 P50/P95 分开报告。
完整命令、冻结编号、结果边界和哈希见 `docs/GRASP_SCORER_TRAINING_CN.md` 与
`evidence/training/grasp-scorer-smoke-rocm-v1.json`。

## 已实现的质量感知适配器边界

几何生成器现在负责完整刚体更新：均匀盒体质量、合并质心与平行轴完整惯量。配置和 CLI 层只传递
显式物理输入，生成资产名称与遥测保留复现所需数值。功能关闭时 stock 行为不变，同时遥测会正确
报告 `stock_explicit_inertia` 和零附加质量。

优化在第一项安全回归处停止：机理恢复通过，但已知成功哨兵可复现地以 38.34 N 中止。这是一项
有价值的负结果，说明提高物理真实性会改变接触分支，并使早期几何结果失效。流程不允许用参数扫描、
扩大活动或模型训练掩盖该失败。完整公式、命令、哈希与下一项允许开展的诊断见
`docs/PARCEL_GRIPPER_ADAPTER_INERTIA_CN.md`。

## 已实现的接触分支诊断边界

默认关闭的诊断在显式控制帧窗口内，以 240 Hz 物理频率记录所有活动机器人接触和两根手指执行器
状态。纯函数汇总轨迹、拒绝重复样本键，并比较接触身份、力、位置、速度、实际力和控制器力的分叉；
它不改变动作、候选排序或 35 N 监督器。

冻结比较显示，原装指尖接触与受力先于手指位置分叉，控制器力从未分叉；质量感知轨迹随后在
203/4 产生 115.5 mm 数值穿透与 162.19 N 冲击。唯一预注册的 0.010 s equality 约束候选已经
确认生效，但更早在第 134 帧以 105.85 N 中止，因此被否决，没有继续扫描。完整双语协议、设置
覆盖修正、结果与产物哈希见 `docs/CONTACT_BRANCH_DIAGNOSTIC_PROTOCOL_CN.md`、
