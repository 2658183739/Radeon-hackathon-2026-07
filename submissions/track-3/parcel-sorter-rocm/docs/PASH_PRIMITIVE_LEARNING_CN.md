...
接线冒烟、2,800 步单 Radeon 训练和冻结 100 回合晋升评估；候选以 94/100 正式通过现有晋升门。
...
| 冻结任务门 | 候选 94/100，基线 96/100，零力越界，正式晋升 |
| 泛化边界 | 仅四类刚性快递参数随机化；未见几何和 Sim-to-Real 未证明 |
...
候选与历史冻结基线按完全相同顺序执行 100 组物理参数。基线为 96/100，2,800 步候选为
94/100；候选 Wilson 95% 区间为 `[0.87523, 0.97221]`。候选满足 80% 目标、5 个百分点不劣性、
Wilson 下界不劣性、零力越界、100/100 回合 VLA 实际执行和 100/100 单张 Radeon/ROCm 来源，
因此现有晋升门返回 `promoted=true`。

完整候选审计、配对与 checkpoint 哈希证明位于
`evidence/mobile_bimanual/primitive_learning_2800step_v1/`。94% 只覆盖四类已支持刚性快递的尺寸、
质量、摩擦和偏移随机化，不覆盖未见几何、柔性/透明快递或 Sim-to-Real。
...
InSight 代码。冻结评估证明受支持参数总体上的任务门通过，但不证明未见物体泛化、VLM 自主发现
缺口或 Sim-to-Real。
