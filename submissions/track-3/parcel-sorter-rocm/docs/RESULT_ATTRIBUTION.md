...
| --- | --- | --- | --- |
| 8/10 successful trajectories | Deterministic scripted expert agent (`ScriptedPickPlaceExpert` + `ClosedLoopSupervisor`) | Runs 10 fixed-seed Genesis episodes using privileged simulator state. An episode is successful only when the supervisor reaches `Stage.COMPLETE`; only records with `_result.json` `success=true` enter the demo. | Scripted-agent demonstration provenance only. Not VLA success. |
...
The demo video contains the eight successful scripted-agent episodes. The
scripted agent reads parcel and robot poses from Genesis and uses a finite-state
...
| 结果 | 控制器 | 结果如何得到 | 可主张内容 |
| --- | --- | --- | --- |
| 8/10 条成功轨迹 | 确定性脚本专家 agent（`ScriptedPickPlaceExpert` + `ClosedLoopSupervisor`） | 使用仿真特权状态运行 10 个固定种子的 Genesis 回合。只有监督器到达 `Stage.COMPLETE` 且 `_result.json` 为 `success=true` 的回合才进入演示视频。 | 仅证明脚本 agent 的示范来源，不是 VLA 成功率。 |
...
演示视频只包含 8 个成功的脚本专家 agent 回合。该 agent 从 Genesis
