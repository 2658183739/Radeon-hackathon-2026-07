...
后续物理真实性实现已经加入固定有效密度、附加质量、质心偏移和合并完整惯量。第一道安全门保留了
`+40 mm` 恢复，却可复现地把 `+45 mm` 成功哨兵变成 38.34 N 中止，因此继续默认关闭，也没有进入
新的评估活动。详见 `docs/PARCEL_GRIPPER_ADAPTER_INERTIA_CN.md`。迁移到实体硬件前，仍必须完成
CAD、材料与紧固件参数辨识、柔顺性、碰撞间隙审查和真实力标定。
