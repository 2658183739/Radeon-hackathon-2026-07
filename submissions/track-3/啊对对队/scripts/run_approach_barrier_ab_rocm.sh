...
OUTPUT_ROOT="${2:-${ROOT_DIR}/outputs/radeon-approach-barrier-ab-v1}"
BARRIER_RECOVERY_STEP_M="${BARRIER_RECOVERY_STEP_M:-0.120}"
EPISODES="${APPROACH_BARRIER_AB_EPISODES:-20}"
...
if [[ "${EPISODES}" != "20" ]]; then
  echo "ERROR: the audited approach-barrier protocol requires exactly 20 episodes" >&2
  exit 4
fi

mkdir -p "${OUTPUT_ROOT}"
BASELINE_ROOT="${OUTPUT_ROOT}/baseline"
CANDIDATE_ROOT="${OUTPUT_ROOT}/candidate-approach-barrier"
...
run_checked "${CANDIDATE_ROOT}/expert/summary.json" \
  ...
  --approach-barrier-recovery-step "${BARRIER_RECOVERY_STEP_M}" \
...
python scripts/compare_expert_runs.py \
  ...
  --allowed-config-difference control.approach_barrier_recovery_step_m \
...
echo "Approach-barrier A/B completed: ${OUTPUT_ROOT}"
