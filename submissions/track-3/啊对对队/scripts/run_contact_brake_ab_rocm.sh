...
OUTPUT_ROOT="${2:-${ROOT_DIR}/outputs/radeon-contact-brake-ab-v1}"
BRAKE_FORCE_N="${BRAKE_FORCE_N:-20.0}"
BRAKE_STEP_M="${BRAKE_STEP_M:-0.010}"
EPISODES="${CONTACT_BRAKE_AB_EPISODES:-20}"
...
if [[ "${EPISODES}" != "20" ]]; then
  echo "ERROR: the audited contact-brake protocol requires exactly 20 episodes" >&2
  exit 4
fi

mkdir -p "${OUTPUT_ROOT}"
BASELINE_ROOT="${OUTPUT_ROOT}/baseline"
CANDIDATE_ROOT="${OUTPUT_ROOT}/candidate-contact-brake"
...
run_checked "${CANDIDATE_ROOT}/expert/summary.json" \
  ...
  --approach-contact-brake-force "${BRAKE_FORCE_N}" \
  --approach-contact-brake-step "${BRAKE_STEP_M}" \
...
python scripts/compare_expert_runs.py \
  ...
  --allowed-config-difference control.approach_contact_brake_force_n \
...
echo "Contact-brake A/B completed: ${OUTPUT_ROOT}"
