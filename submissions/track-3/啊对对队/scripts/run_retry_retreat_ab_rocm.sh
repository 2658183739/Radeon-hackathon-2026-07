...
OUTPUT_ROOT="${2:-${ROOT_DIR}/outputs/radeon-retry-retreat-ab-v1}"
RETREAT_DISTANCE_M="${RETRY_RETREAT_DISTANCE_M:-0.120}"
EPISODES="${RETRY_RETREAT_AB_EPISODES:-20}"
...
if [[ "${EPISODES}" != "20" ]]; then
  echo "ERROR: the audited retry-retreat protocol requires exactly 20 episodes" >&2
  exit 4
fi

mkdir -p "${OUTPUT_ROOT}"
BASELINE_ROOT="${OUTPUT_ROOT}/baseline"
CANDIDATE_ROOT="${OUTPUT_ROOT}/candidate-retry-retreat"
...
run_expert_checked "${CANDIDATE_ROOT}/expert/summary.json" \
  ...
  --start-episode 0 \
  --profile "${PROFILE}" \
  --retry-retreat-distance "${RETREAT_DISTANCE_M}" \
...
python scripts/compare_expert_runs.py \
  ...
  --allowed-config-difference task.retry_retreat_distance_m \
...
echo "Retry-retreat A/B completed: ${OUTPUT_ROOT}"
