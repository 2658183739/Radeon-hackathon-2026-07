...
OUTPUT_ROOT="${2:-${ROOT_DIR}/outputs/radeon-approach-step-ab-v1}"
APPROACH_STEP_M="${APPROACH_STEP_M:-0.020}"
EPISODES="${APPROACH_STEP_AB_EPISODES:-20}"
...
if [[ "${EPISODES}" != "20" ]]; then
  echo "ERROR: the audited approach-step protocol requires exactly 20 episodes" >&2
  exit 4
fi

mkdir -p "${OUTPUT_ROOT}"
BASELINE_ROOT="${OUTPUT_ROOT}/baseline"
CANDIDATE_ROOT="${OUTPUT_ROOT}/candidate-approach-step"
...
python scripts/compare_expert_runs.py \
  ...
  --allowed-config-difference control.approach_step_m \
...
echo "Approach-step A/B completed: ${OUTPUT_ROOT}"
