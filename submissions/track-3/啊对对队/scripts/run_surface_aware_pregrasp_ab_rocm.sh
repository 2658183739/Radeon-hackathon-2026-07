...
OUTPUT_ROOT="${2:-${ROOT_DIR}/outputs/radeon-surface-aware-pregrasp-ab-v1}"
EPISODES="${SURFACE_AWARE_PREGRASP_AB_EPISODES:-20}"
...
if [[ "${EPISODES}" != "20" ]]; then
  echo "ERROR: the audited surface-aware pregrasp protocol requires exactly 20 episodes" >&2
  exit 3
fi

mkdir -p "${OUTPUT_ROOT}"
BASELINE_ROOT="${OUTPUT_ROOT}/baseline-collision-checked-reset"
CANDIDATE_ROOT="${OUTPUT_ROOT}/candidate-surface-aware-pregrasp"
...
run_checked "${CANDIDATE_ROOT}/expert/summary.json" \
  --config "${CONFIG_PATH}" --backend rocm --episodes "${EPISODES}" \
  --start-episode 0 --profile "${PROFILE}" --collision-checked-reset \
  --surface-aware-pregrasp --output "${CANDIDATE_ROOT}" \
...
python scripts/compare_expert_runs.py \
  ...
  --allowed-config-difference task.surface_aware_pregrasp_enabled \
...
echo "Surface-aware pregrasp A/B completed: ${OUTPUT_ROOT}"
