...
OUTPUT_ROOT="${2:-${ROOT_DIR}/outputs/radeon-approach-velocity-ab-v1}"
EPISODES="${APPROACH_VELOCITY_AB_EPISODES:-20}"
...
if [[ "${EPISODES}" != "20" ]]; then
  echo "ERROR: the audited approach-velocity protocol requires exactly 20 episodes" >&2
  exit 4
fi

mkdir -p "${OUTPUT_ROOT}"
BASELINE_ROOT="${OUTPUT_ROOT}/baseline"
CANDIDATE_ROOT="${OUTPUT_ROOT}/candidate-velocity"
...
run_checked "${CANDIDATE_ROOT}/expert/summary.json" \
  --config "${CONFIG_PATH}" --backend rocm --episodes "${EPISODES}" \
  --start-episode 0 --profile "${PROFILE}" --approach-velocity-control \
...
python scripts/compare_expert_runs.py \
  ...
  --allowed-config-difference control.approach_velocity_control_enabled \
...
python scripts/compact_expert_summary.py \
  --summary "${CANDIDATE_ROOT}/expert/summary.json" \
  --output "${OUTPUT_ROOT}/candidate-velocity-summary.json" --overwrite
...
sha256sum \
  ...
  "${OUTPUT_ROOT}/candidate-velocity-summary.json" \
  "${OUTPUT_ROOT}/comparison.json" \
  "${OUTPUT_ROOT}/baseline_failure_analysis.json" \
  "${OUTPUT_ROOT}/candidate_failure_analysis.json" \
  "${OUTPUT_ROOT}/baseline-run.log" \
...
echo "Approach-velocity A/B completed: ${OUTPUT_ROOT}"
