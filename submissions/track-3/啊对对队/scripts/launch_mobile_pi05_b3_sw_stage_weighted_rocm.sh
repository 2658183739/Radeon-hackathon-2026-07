...
LOG_PATH="${3:-${PERSISTENCE_ROOT}/outputs/pi05-b3-sw-stage-weighted-train-v1.log}"
...
test -f "${DATASET_ROOT}/PI05_ABSOLUTE_DATASET_MANIFEST.json" || {
  echo "ERROR: B3-SW requires the frozen absolute-action dataset" >&2
  exit 2
}
if [[ -e "${OUTPUT_DIR}" || -e "${LOG_PATH}" || -e "${PID_PATH}" || -e "${TELEMETRY_DIR}" ]]; then
  echo "ERROR: B3-SW output, log, PID, or telemetry already exists" >&2
  exit 2
fi
if pgrep -f "lerobot_eval.py .*--policy.device=cuda|lerobot_train|train_mobile_pi05_rocm.sh" >/dev/null; then
  echo "ERROR: another PI0.5 GPU task is running; B3-SW refuses to contend for Radeon" >&2
  exit 3
fi
...
if ! kill -0 "${training_pid}" 2>/dev/null; then
  echo "ERROR: B3-SW training exited during launch; inspect ${LOG_PATH}" >&2
  exit 4
fi
printf '{"status":"started","candidate":"B3-SW","pid":%s,"dataset":"%s","output":"%s","log":"%s","rocm_telemetry":"%s"}\n' \
