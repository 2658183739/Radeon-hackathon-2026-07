...
OUTPUT_ROOT="${2:-${ROOT_DIR}/outputs/radeon-approach-compliance-ab-v1}"
APPROACH_STIFFNESS_SCALE="${APPROACH_STIFFNESS_SCALE:-0.500}"
EPISODES="${APPROACH_COMPLIANCE_AB_EPISODES:-20}"
...
if [[ "${EPISODES}" != "20" ]]; then
  echo "ERROR: the audited approach-compliance protocol requires exactly 20 episodes" >&2
  exit 4
fi

mkdir -p "${OUTPUT_ROOT}"
BASELINE_ROOT="${OUTPUT_ROOT}/baseline"
CANDIDATE_ROOT="${OUTPUT_ROOT}/candidate-compliance"
...
run_checked "${CANDIDATE_ROOT}/expert/summary.json" \
  ...
  --approach-stiffness-scale "${APPROACH_STIFFNESS_SCALE}" \
...
python scripts/compare_expert_runs.py \
  ...
  --allowed-config-difference control.approach_stiffness_scale \
...
python scripts/compact_expert_summary.py \
  --summary "${CANDIDATE_ROOT}/expert/summary.json" \
  --output "${OUTPUT_ROOT}/candidate-compliance-summary.json" --overwrite
...
sha256sum \
  ...
  "${OUTPUT_ROOT}/candidate-compliance-summary.json" \
  "${OUTPUT_ROOT}/comparison.json" \
  "${OUTPUT_ROOT}/baseline_failure_analysis.json" \
  "${OUTPUT_ROOT}/candidate_failure_analysis.json" \
  "${OUTPUT_ROOT}/baseline-run.log" \
...
echo "Approach-compliance A/B completed: ${OUTPUT_ROOT}"
