...
OUTPUT_ROOT="${2:-${ROOT_DIR}/outputs/radeon-collision-checked-reset-ab-v1}"
EPISODES="${COLLISION_CHECKED_RESET_AB_EPISODES:-20}"
...
if [[ "${EPISODES}" != "20" ]]; then
  echo "ERROR: the audited collision-checked reset protocol requires exactly 20 episodes" >&2
  exit 4
fi

mkdir -p "${OUTPUT_ROOT}"
BASELINE_ROOT="${OUTPUT_ROOT}/baseline"
CANDIDATE_ROOT="${OUTPUT_ROOT}/candidate-collision-checked-reset"
...
echo "Collision-checked reset A/B completed: ${OUTPUT_ROOT}"
